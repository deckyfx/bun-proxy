import type { WhitelistEntry, WhitelistOptions, WhitelistStats } from './BaseDriver';
...
export class OptimizedFileDriver extends BaseDriver {
  ...
  private walEntries = new Map<string, { action: 'add' | 'remove', entry?: WhitelistEntry }>();
  ...
  private async ensureIndexLoaded(): Promise<void> {
    ...
    try {
      ...
      if ((error as any)?.code !== 'ENOENT') {
        console.warn('Failed to load whitelist index:', error);
      }
  ...
  private async compactWal(): Promise<void> {
    try {
      ...
      try {
        ...
        const mainData = JSON.parse(mainContent) as WhitelistEntry[];
  ...
  private async rebuildIndex(): Promise<void> {
    try {
      ...
      const data = JSON.parse(content) as WhitelistEntry[];
